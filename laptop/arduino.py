...
def play_on_arduino(data):
    ...
    try:
        ...
        log("Afspelen succesvol voltooid!")
    except KeyboardInterrupt:
        log("\nAfspelen handmatig afgebroken.")
    except Exception as e:
        log(f"Fout tijdens sturen naar Arduino: {e}")


# ---------------------------------------------------------
# MIDI PARSING FUNCTIES (Ongewijzigd gebleven)
# ---------------------------------------------------------
...
def parse_track(track_data, track_index):
    ...
    while offset < len(track_data):
        ...
        if event_type == 0x90:
            ...
            events.append((abs_tick, track_index, event_index, SimpleNamespace(type="note_on", note=data1, velocity=velocity, channel=channel, time=0)))
            event_index += 1
        elif event_type == 0x80:
            events.append((abs_tick, track_index, event_index, SimpleNamespace(type="note_off", note=data1, velocity=data2 or 0, channel=channel, time=0)))
            event_index += 1
...
def parse_midi_file(path):
    ...
    # Sorteer alle events op basis van absolute ticks in de MIDI
    all_events.sort(key=lambda item: (item[0], item[1], item[2]))

    # --- NIEUW: Ticks omrekenen naar échte milliseconden ---
    merged = []
    previous_tick = 0
    current_tempo = 500000  # Default: 120 BPM (500.000 microseconden per kwartnoot)
    abs_time_ms = 0.0

    for abs_tick, _, _, event in all_events:
        delta_ticks = abs_tick - previous_tick
        
        # Bereken de werkelijke verstreken tijd in MS met het huidige tempo en divisie
        abs_time_ms += (delta_ticks * current_tempo) / (division * 1000.0)
        
        # Sla tempo veranderingen op voor de berekening van komende noten
        if event.type == "set_tempo":
            current_tempo = event.tempo
            
        event.abs_time_ms = abs_time_ms
        previous_tick = abs_tick
        merged.append(event)

    # Bereken nu de 'delay' (milliseconde-verschil) ten opzichte van het vorige event
    final_merged = []
    prev_ms = 0
    for event in merged:
        ms = int(round(event.abs_time_ms))
        event.time = max(0, ms - prev_ms) # Overschrijf de verouderde "ticks" met echte MS
        prev_ms = ms
        final_merged.append(event)
...
def messages_to_table(messages):
    ...
    for message in messages:
        if message.type == "set_tempo":
            # Tempo wordt door de Python code in milliseconden omgezet,
            # dus we sturen naar de Arduino alleen nog maar een "blanco" delay instructie.
            table.append([NUM_SPEAKERS + 1, 0, message.time])
            continue
    ...
    # Zorg dat alles stopt als het liedje klaar is
...
def convert_midi_file(input_path):
    try:
        ...
        log(f"Bestand succesvol verwerkt: {input_path.name}")
        return output_path
    except Exception as e:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Converteer en stuur een MIDI-bestand direct naar je Arduino via Serial."
    )
    # File is nu optioneel gemaakt. Als je het weglaat, opent er een venster.
    parser.add_argument("file", nargs="?", default=None, help="Pad naar het .mid of .midi bestand (optioneel)")
    parser.add_argument("--port", default="COM10", help="Arduino COM poort (default: COM10)")
...
def main():
    ...
    # 1. Bepaal welk bestand we gaan gebruiken
    if not file_path:
        # Open een pop-up venster om een bestand te kiezen!
        root = tk.Tk()
        root.withdraw() # Verberg het hoofdscherm van tkinter
        file_path = filedialog.askopenfilename(
            title="Selecteer een MIDI bestand",
            filetypes=[("MIDI Files", "*.mid *.midi")]
        )
        
        if not file_path: # Als de gebruiker op "Annuleren" klikt
            log("Geen bestand geselecteerd. Programma stopt.")
            return 0
            
    input_path = Path(file_path)

    if not input_path.exists():
        log(f"Fout: Bestand '{input_path}' niet gevonden.")
        return 1

    if input_path.suffix.lower() not in INPUT_EXTENSIONS:
        log("Fout: Alleen .mid of .midi bestanden zijn toegestaan.")
        return 1

    # 2. Converteer het bestand
    try:
        txt_path = convert_midi_file(input_path)
        txt_content = txt_path.read_text(encoding='utf-8')
    except Exception as e:
        log(f"Fout bij voorbereiden data: {e}")
        return 1
    ...
    try:
        ...
        log(f"Kan niet afspelen: {e}")
        return 1

    # 4. Speel af
    try:
        play_on_arduino(txt_content)
    finally:
        if serial_conn and serial_conn.is_open:
            serial_conn.close()
            log("Seriële verbinding gesloten.")

...
class _PlayPageState extends State<PlayPage> {
  ...
  String get _intentDescription {
    return widget.intent == PlayIntent.game
        ? 'Kies een song of upload je eigen MIDI om meteen als tiles-game te spelen.'
        : 'Kies een song of upload je eigen MIDI om direct te spelen.';
  }
...
class _GamePageState extends State<GamePage> {
  ...
  late double _timingOffsetMillis;
  ...
  void initState() {
    super.initState();
    _timingOffsetMillis = widget.difficulty.syncOffsetMillis;
    _initGame();
  }
  ...
  Future<void> _openTimingCalibration() async {
    if (_isPlaying) {
      return;
    }

    final double? selectedOffset = await showDialog<double>(
      context: context,
      builder: (BuildContext dialogContext) {
        double currentValue = _timingOffsetMillis;

        return StatefulBuilder(
          builder: (BuildContext context, void Function(void Function()) setState) {
            return AlertDialog(
              title: const Text('Timing afstellen'),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Text('Offset: ${currentValue.round()} ms'),
                  Slider(
                    value: currentValue,
                    min: -250,
                    max: 250,
                    divisions: 100,
                    label: '${currentValue.round()} ms',
                  ...
                  const Text(
                    'Negatief = eerder, positief = later. Dit verschuift zowel de muziek als de hit-timing.',
                  ),
                ],
              ),
              actions: <Widget>[
                ...
                TextButton(
                  onPressed: () {
                    setState(() {
                      currentValue = widget.difficulty.syncOffsetMillis;
                    });
                  },
    ...
    setState(() {
      _timingOffsetMillis = selectedOffset;
    });
  }

  double _effectiveStartMillis(_GameTileNote note) {
    return note.startMillis + _timingOffsetMillis;
  }
  ...
  void _processTrackAudio(double elapsedMillis) {
    ...
    while (_audioCursor < parsedSong.notes.length) {
      ...
      final double startMillis =
          (note.startMicros / 1000) + _timingOffsetMillis;
                    ...
                    Padding(
                      ...
                      child: Row(
                        children: <Widget>[
                          ...
                          IconButton(
                            tooltip: 'Timing afstellen',
                            onPressed: _isPlaying
                                ? null
                                : _openTimingCalibration,
